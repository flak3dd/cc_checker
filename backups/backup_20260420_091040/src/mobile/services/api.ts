...
export const api = {
  ...
  getGateway2CrawlStatus: async (): Promise<{ is_running: boolean }> => {
    const { data } = await apiClient.get('/api/gateway2/crawl/status');
    return data;
  },
  ...
  startGateway2Crawl: async (): Promise<{ success: boolean; message: string }> => {
    const { data } = await apiClient.post('/api/gateway2/crawl/start');
    return data;
  },
  ...
  stopGateway2Crawl: async (): Promise<{ success: boolean; message: string }> => {
    const { data } = await apiClient.post('/api/gateway2/crawl/stop');
    return data;
  },
