...
export default function DashboardScreen() {
  ...
  const { data: crawlStatus, isLoading: crawlLoading, refetch: refetchCrawl } = useGateway2CrawlStatusQuery();
  ...
  const filteredResults = useMemo(() => {
    let filtered = results.filter(r =>
      (r.card_number && String(r.card_number).includes(search)) ||
      (r.plate && String(r.plate).includes(search)) ||
      (r.status && String(r.status).toLowerCase().includes(search.toLowerCase()))
    );
  ...
  const crRunning = !!crawlStatus?.is_running;
  const activeCount = [ccRunning, coRunning, plRunning, gwRunning, crRunning].filter(Boolean).length;
  ...
  const activeLabels = useMemo(() => {
    ...
    if (crRunning) labels.push('Crawl');
    return labels;
  }, [ccRunning, coRunning, plRunning, gwRunning, crRunning]);
  ...
  const heroData = useMemo(() => ({
    ...
    analytics,
  }), [passCount, failCount, successRate, totalChecked, status, checkoutStatus, analytics]);
            ...
            <AnimatedCard index={2}>
              {isRefreshing ? (
                <>
                  ...
                  <HeroStats passCount={0} failCount={0} successRate={0} queueCount={0} totalChecked={0} waPlates={0} analytics={{}} />
                </>
              ) : (
                <>
              ...
              <AnimatedCard index={6} style={styles.controlCard}>
                ...
                >
                  <View style={styles.crawlRow}>
                    <Text style={[styles.controlHint, { flex: 1 }]}>Validates cards via DonorPerfect donation gateway</Text>
                    <Button
                      mode={crRunning ? "contained" : "outlined"}
                      onPress={() => crRunning 
                        ? execute(() => api.stopGateway2Crawl(), 'Crawl stopped', 'Failed to stop', handleRefresh)
                        : execute(() => api.startGateway2Crawl(), 'Crawl started', 'Failed to start', handleRefresh)
                      }
                      compact
                      loading={crawlLoading}
                      style={[styles.crawlBtn, crRunning && { backgroundColor: colors.danger }]}
                      labelStyle={styles.crawlBtnLabel}
                    >
                      {crRunning ? "STOP CRAWL" : "CRAWL"}
                    </Button>
                  </View>
                </ControlPanel>
              </AnimatedCard>
              ...
              {isRefreshing ? (
                ...
                <LiveLogPanel
                  tabs={[
                    ...
                    { file: 'gateway2-crawl', title: 'GW2 Crawl', icon: 'search' },
                  ]}
                  height={350}
                />
              )}
