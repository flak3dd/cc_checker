...
async function processCard(page: Page, cardData: string): Promise<string> {
  ...
  try {
    ...
    const successSelector = "text=/Thank you/i, text=/Successful/i, text=/received/i";
    const failSelector = "text=/declined/i, text=/failed/i, text=/error/i, text=/invalid/i, .error, .alert-danger";
    ...
    const pageText = await page.innerText('body');
    const failPatterns = ["declined", "failed", "invalid card", "insufficient funds", "error", "expired", "not authorized"];
...
async function main() {
  ...
  while (true) {
    ...
    const currentCard = lines[0].trim();
    ...
    fs.writeFileSync(targetFile, remaining.join('\n'));
    logToFile(`Processed ${currentCard.split('|')[0].slice(-4)} -> ${status}. ${remaining.length} left.`);
